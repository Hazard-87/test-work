...
const Content = () => {
  ...
  const fetchImage = async (i) => {
    let response = await axios.get(
      `https://api.giphy.com/v1/gifs/random?api_key=${API_KEY}&tag=${text[i]}`,
    );
    arr = [
      ...arr,
      {
  ...
  const loadImages = () => {
    try {
      if (text[0] === 'delay') {
        ...
        let delay = 5000;
        while (i < 6) {
          setTimeout((word) => fetchImage(word), delay * i);
          i++;
        }
      } else if (text) {
        setLoading(true);
    ...
    <div>
      <header className={'header'}>
        ...
        {loading ? (
          ...
          <Button onClick={loadImages} variant="success">
            Загрузить
          </Button>
        )}
        ...
        {!group ? (
          <Button onClick={onGroup} variant="primary">
            Группировать
          </Button>
        ) : (
          <Button onClick={onGroup} variant="primary">
        ...
        {group ? (
          ...
          <div className={'content'}>
            {images.map((image, index) => (
              <img
                alt={'photo'}
                onClick={() => setText(image.categories)}
                key={index}
                src={image.url}
              />
            ))}
